...
const MyInformation = ({ dispatch }) => {
	...
	const updateUser = () => {
		updateCollection('plant_usuarios', newuser.id, newuser).then((r) => {
			r ? alert('Cambios realizados con éxito') : alert('Ups, sucedió un error');
		});
			...
			<View style={{ justifyContent: 'center', flex: 5 }}>
				...
				{isEditable ? (
					<TextInput
						...
						placeholder={field === 'name' ? 'Nombre y Apellido' : 'Numero de celular'}
					...
					title === 'Email' && (
						<Text style={{ fontSize: RFPercentage(1.9) }} onPress={onShowCalendar}>
							{form[field]}
						</Text>
					)
				)}
