...
const Product = (props) => {
	...
	useEffect(() => {
		const actualizarRuta = (ruta) => dispatch(actions.actualizarUbicacion(ruta));
		actualizarRuta(routeName);

		setLoading(true);
		const getProducts = async () =>
			await getCollection('plant_productos').then((response) => dispatch(actions.fetchProducts(response)));

		getProducts().then(() => setLoading(false));

		registerForPushNotificationsAsync().then((token) => dispatch(actions.setPushToken(token)));
	}, []);
	...
	return (
		<Container>
			...
			/>
			<ScrollView style={{ height: '100%', width: '100%' }} contentContainerStyle={[{ paddingVertical: 5 }]}>
							...
							<ScrollView horizontal style={{ alignSelf: 'center' }}>
								{products
									...
									.map((value, index) => {
										return (
											<TouchableOpacity
												onPress={() => navigation.navigate('ProductInfo', { item: value })}
												key={index}
										...
										return (
											...
											>
												<Image
													source={{ uri: value.product.photo_url }}
													style={{
														width: pantalla.screenHeight <= 592 ? wp(15) : wp(18),
														height: hp(22),
													}}
													resizeMode="contain"
												/>
